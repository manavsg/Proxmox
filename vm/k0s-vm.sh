...
TEMP_DIR=$(mktemp -d)
pushd $TEMP_DIR >/dev/null
if whiptail --backtitle "Proxmox VE Helper Scripts" --title "k0s VM" --yesno "This will create a New k0s VM. Proceed?" 10 58; then
  :
else
...
function default_settings() {
  ...
  echo -e "${BL}Creating a k0s VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 k0s --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="k0s"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a k0s VM?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating a k0s VM using the above advanced settings${CL}"
  else
    header_info
